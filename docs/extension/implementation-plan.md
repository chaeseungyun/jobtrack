...
| 개발 | `localhost:3000` | 로컬 개발 시 수동으로 manifest 수정하여 사용 |

> **manifest.json의 `host_permissions`와 `content_scripts`에는 프로덕션 도메인을 기본값으로 사용한다.** 로컬 개발 시에는 `http://localhost:3000/*`을 임시로 추가하여 테스트한다.
