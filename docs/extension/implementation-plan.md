...
| 환경 | 도메인 | 용도 |
|------|--------|------|
| 프로덕션 | `jobtrack-phi.vercel.app` | Chrome Web Store 배포용 manifest |
| 스테이지 | `jobtrack-70l6elklc-chaeseungyuns-projects.vercel.app` | PR 프리뷰 테스트 |
| 개발 | `localhost:3000` | 로컬 개발 시 확장 프로그램 연동 테스트 |

> 확장 프로그램 코드에는 `DEV_WEB_ORIGIN`, `PROD_WEB_ORIGIN`, `DEFAULT_WEB_ENV` 상수를 둔다. 인증 시작 전에 `apiBase`를 초기화하고, 현재 작업 단계에서는 `DEFAULT_WEB_ENV = "development"`로 로컬 테스트를 우선한다. 배포 패키징 전에는 `production` 기준으로 다시 확인한다.
...
> **설계 원칙**: Service Worker 없이 2-컴포넌트 구조. API 호출은 popup에서 직접 수행한다. 인증 토큰 저장도 content_script(`auth-callback.js`)가 `chrome.storage.local`에 직접 저장한다. Content script는 `chrome.storage` API에 접근 가능하므로 Service Worker를 경유할 필요가 없다.
...
- 웹 로그인 완료 후 리다이렉트되는 **클라이언트 컴포넌트** 페이지
- 클라이언트에서 `fetch('/api/auth/extension-token')` 호출하여 토큰 발급 (사용자가 직접 로그인한 상태이므로 세션 쿠키가 브라우저에 존재)
- 확장 프로그램이 이 페이지에 주입한 content_script를 통해 토큰을 전달받음
- 플로우:
  1. 확장 프로그램 → `/auth?from=extension` 새 탭 열기
  2. 사용자 로그인 완료
  3. `/auth/extension-callback`으로 리다이렉트
  4. 클라이언트 컴포넌트가 마운트 시 `/api/auth/extension-token` fetch 호출 (쿠키 자동 첨부)
  5. 응답받은 토큰을 `data-extension-token` 속성으로 DOM에 삽입
  6. content_script(`auth-callback.js`)가 `MutationObserver`로 `data-extension-token` 속성 삽입을 감지하여 토큰을 읽음
     - content_script는 `document_idle` 시점에 실행되므로, 클라이언트 컴포넌트의 fetch 완료보다 먼저 실행됨 → 반드시 MutationObserver로 대기 필요
  7. content_script가 `chrome.storage.local`에 토큰 + 만료일 직접 저장 (Service Worker 불필요)
  8. 콜백 페이지에 **"로그인 완료! 확장 프로그램 아이콘을 다시 클릭해주세요"** 안내 문구 표시
     - 팝업은 새 탭이 열리면서 포커스를 잃어 자동으로 닫힘
     - 사용자가 다시 아이콘을 클릭하면 팝업 Init → storage에서 토큰 확인 → 메인 UI 진입
...
│   ├── popup.html
│   ├── popup.css
│   └── popup.js              # 팝업 UI 로직 + API 호출
├── content/
│   ├── extractor.js          # 공고 페이지 HTML 추출 (셀렉터 기반 정제)
│   └── auth-callback.js      # 인증 콜백 페이지 토큰 저장
├── utils/
│   ├── api.js                # API 호출 래퍼
...
```json
{
  ...
  "content_scripts": [
    {
      "matches": [
        "https://jobtrack-phi.vercel.app/auth/extension-callback*"
      ],
...
사용자                  팝업                    JobTrack 웹 (콜백 페이지)
  ...
  │── 로그인 클릭 ──────→│                            │
  │                     │── apiBase 초기화            │
  │                     │── 새 탭 열기 ──────────────→│ /auth?from=extension
  │                     │ (팝업 자동 닫힘)            │
  │                     │                            │── 로그인 폼
  │── 로그인 완료 ──────────────────────────────────→│
  │                     │                            │── /auth/extension-callback
  │                     │                            │── 클라이언트 fetch → 토큰 발급
  │                     │                            │── DOM에 data-extension-token 삽입
  │                     │                            │── content_script: MutationObserver로 감지
  │                     │                            │── content_script: chrome.storage.local에 직접 저장
  │                     │                            │   (실제 콜백 origin으로 apiBase 동기화)
  │                     │                            │── "로그인 완료! 아이콘을 다시 클릭해주세요"
  │── 아이콘 재클릭 ───→│                            │
  │                     │── storage 토큰 확인 → 유효  │
  │                     │── 메인 UI 표시             │
```
...
MV3 `content_scripts`는 `matches` 패턴에 해당하는 **document 풀페이지 로드 시점에만 주입**된다. `/auth/extension-callback`으로 이동할 때 Next.js `router.push/replace` 같은 SPA soft-nav(`history.pushState`)를 사용하면 content script가 실행되지 않아 토큰이 `chrome.storage.local`에 저장되지 않고, 사용자가 새로고침을 해야 비로소 동작한다.

강제 지점:
- `src/app/auth/_components/auth-form.client.tsx`의 `onSuccess` 핸들러에서 `callbackUrlOverride`(= 확장 플로우 시그널)가 있을 때는 `window.location.assign(callbackUrl)`로 풀로드를 강제한다.
- `src/app/auth/page.tsx`의 기 로그인 분기(`redirect("/auth/extension-callback")`)는 서버 side redirect이므로 풀로드를 보장하여 문제없음.

향후 AuthForm 리팩토링 시 이 분기를 제거해서는 안 된다. NFR-1a 참조.

#### 3-2. 공고 저장 플로우

> Popup이 API를 직접 호출한다. Service Worker는 사용하지 않는다.
...
- 모든 API 호출은 **popup에서 직접** 수행
- `getApiBase()`는 storage 값을 우선 사용하고, 없으면 환경 상수(`DEFAULT_WEB_ENV`) 기반 초기값을 사용
- 로그인 시작 전 `ensureApiBase()`로 origin을 먼저 저장하여 첫 인증부터 로컬/프로덕션을 명확히 분기
- 토큰 만료 감지 → 401 응답 시 로그인 상태 초기화 + 재로그인 유도
- 기본 헤더: `Authorization: Bearer <token>`, `Content-Type: application/json`
- 반환값은 `errorType: "http" | "parse" | "network" | "auth"`로 실패 원인을 구분
...
| API 통신 | Fetch API (popup에서 직접) | 추가 의존성 불필요 |
...
| Service Worker 미사용에 따른 제약 | 없음 | content_script가 `chrome.storage.local`에 직접 접근 가능하므로 Service Worker 불필요. 향후 필요 시 추가 가능 |
