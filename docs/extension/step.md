...
- [ ] 공통 에러 표시 유틸 도입 — `showError(view, message, retryFn?)`가 `#site-status` 갱신 + 재시도 버튼 토글
- [ ] 파싱/저장 경로의 `catch`에서 `errorType` 분기
...
- [ ] 타임아웃 보조 안내 — 파싱 호출 5초 경과 시 "시간이 걸리고 있습니다..." 텍스트 표시 (`AbortController`는 도입하지 않음)
- [ ] 재시도 버튼은 기존 뷰에 인라인 추가 (별도 에러 전용 뷰 없음)
