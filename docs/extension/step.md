...
└── operations.md            # 배포 후 어떻게 운영할 것인가 (릴리스/관측/장애 대응)
```
...
Step 1 (Bearer 인증)
  ...
  │                 └── Step 7-1 (에러 통합) → 7-2 (엣지 케이스) → 7-3 (UI 폴리싱)
...
- [x] 사람인 공고 → 저장하기 → 파싱 → 확인 화면 → 폼 자동 채움 → 저장 → 성공 화면 E2E 통과
- [x] 잡코리아 동일 E2E 통과
- [x] JobTrack 웹(`/applications`)에서 저장된 지원서 확인
...
Step 6의 핵심 플로우를 안정화하는 마무리 단계. 세 개의 서브 Step으로 분할하여 각각 독립된 대화에서 완료 가능하도록 구성.

- **Step 7-1**: 에러 상태 통합 처리 (기능적 완성도)
- **Step 7-2**: 엣지 케이스 처리 (견고성)
- **Step 7-3**: UI 디자인 톤 맞춤 (시각 폴리싱)

---

### Step 7-1: 에러 상태 통합 처리

**목표**: `api.js`가 이미 반환하는 `errorType`을 popup에서 분기 처리하여 모든 실패 경로에 사용자 안내와 복구 동작을 부여한다.
...
| 읽을 파일 | 이유 |
|-----------|------|
| `docs/extension/requirements.md` § 4 | NFR-5(타임아웃), NFR-8(만료 재로그인), NFR-10(에러 타입) |
| `docs/extension/flow.md` § 5 | Error / SaveError 상태 전이 |
| `extension/utils/api.js` | 반환 `errorType: auth|http|parse|network` 구조 확인 |
| `extension/utils/auth.js` | `clearToken()` 재사용 지점 |
| `extension/popup/popup.js` | 현재 에러 분기 미활용 지점 (line 127 부근) |

#### 작업

- [x] 공통 에러 표시 유틸 도입 — `showError({ context, errorType })`가 `#site-status` / `#form-error` 갱신, 기존 `#btn-save` / `#btn-submit`을 인라인 재시도로 재사용
- [x] 파싱/저장 경로의 `catch`에서 `errorType` 분기
  - `network` → "연결할 수 없습니다. 인터넷 연결을 확인해주세요."
  - `auth` → `clearToken()` 호출 후 `view-login`으로 전환
  - `http` / `parse` (파싱 경로) → "분석에 실패했습니다. 다시 시도해주세요." + 재시도 버튼
  - `http` / `parse` (저장 경로) → "저장에 실패했습니다." + 재시도 버튼 (폼 입력 유지)
- [x] 타임아웃 보조 안내 — 파싱 호출 5초 경과 시 "시간이 걸리고 있습니다..." 텍스트 표시 (`AbortController`는 도입하지 않음)
- [x] 재시도 버튼은 기존 뷰에 인라인 추가 (별도 에러 전용 뷰 없음)

#### 완료 조건

- [ ] 오프라인 상태에서 저장 시도 → 네트워크 안내 표시
- [ ] 만료/위조 토큰으로 저장 시도 → 로그인 화면 복귀
- [ ] 서버가 500 반환 시 재시도 버튼 동작
- [x] `pnpm build` 성공
...
**목표**: 비정상 입력·상태에서 확장이 조용히 실패하거나 무한 대기에 빠지지 않도록 방어.
...
| 읽을 파일 | 이유 |
|-----------|------|
| `extension/popup/popup.js` | 팝업 초기화 루틴 및 `state` 리셋 지점 |
| `extension/utils/extractor.js` | 셀렉터 실패 시 반환 형태 및 HTML 크기 측정 위치 |
| `src/app/api/applications/route.ts` | 동일 URL 중복 저장 시 서버 응답 (409 여부 확인) |

#### 작업

- [ ] 팝업 재열기 상태 초기화 — DOMContentLoaded마다 `state` 리셋이 완전한지 검증, 불필요한 뷰 이월 제거
- [ ] 동일 URL 중복 저장 안내
  - 서버 409 응답 시 "이미 저장된 공고입니다" + `link-view` 링크 표시 (서버가 409를 보내지 않으면 Step 7-2 범위에서 서버 측 변경은 제외하고 해당 서브태스크만 보류)
- [ ] HTML 5MB 초과 방어 — 추출 직후 `new Blob([html]).size` 검사, 초과 시 서버 호출 생략하고 안내 표시
- [ ] content 셀렉터 전부 실패 시 안내 — `extractor.js` 빈 결과 케이스에서 "공고 본문을 찾지 못했습니다" 표시

#### 완료 조건

- [ ] 팝업 닫고 재열 때 이전 상태 이월 없음
- [ ] 중복 URL 저장 시나리오에서 안내 메시지 표시 (서버 지원 여부에 맞춰)
...
### Step 7-3: UI 디자인 톤 맞춤

**목표**: 기능·견고성이 확정된 뒤 시각 톤을 JobTrack 웹과 일치시켜 마감.

**선행**: Step 7-1, Step 7-2

#### 컨텍스트 복구

| 읽을 파일 | 이유 |
|-----------|------|
| `src/app/globals.css` | 웹 디자인 토큰 (oklch) 참조 |
| `extension/popup/popup.css` | 현재 자체 토큰 — 값 교체 대상 |
| `extension/popup/popup.html` | 클래스 추가/정리 지점 |

#### 작업

- [ ] 색상 토큰 정렬 — 웹의 `--primary`, `--background`, `--destructive`, `--muted`, `--border` 값을 extension CSS 변수로 하드코딩 미러링 (import 불가)
- [ ] 폰트·간격·라운드 값 통일 (웹의 `--radius`, spacing 기준)
- [ ] 로딩 스피너 스타일 정리
- [ ] 에러/성공 표시 스타일 (인라인 유지, 색·패딩만 정리 — 토스트 도입 안 함)
- [ ] 폼 필드 여백, 포커스 링, 버튼 상태

#### 완료 조건

- [ ] 전체 통합 수동 테스트 통과 (로그인 → 저장 → 확인 E2E)
- [ ] 팝업 UI가 웹 `/auth`, `/applications` 화면과 시각적 이질감 없음
- [ ] `pnpm build` 성공
